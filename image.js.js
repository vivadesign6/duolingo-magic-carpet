...
document.addEventListener('DOMContentLoaded', function() {
    // Replace missing images with generated SVGs
    document.querySelectorAll('img').forEach(img => {
        img.addEventListener('error', function() {
            const imagePath = this.getAttribute('src');
            if (imagePath) {
                const imagePathParts = imagePath.split('/');
                const imageFile = imagePathParts[imagePathParts.length - 1];
                const imageDir = imagePathParts[imagePathParts.length - 2];
                const imagePath = imageDir ? `${imageDir}/${imageFile}` : imageFile;
                
                // Create SVG data URL
                const svgContent = window.createPlaceholderImage(imagePath);
                const svgBase64 = btoa(svgContent);
                this.src = `data:image/svg+xml;base64,${svgBase64}`;
            }
        });
    });
});
